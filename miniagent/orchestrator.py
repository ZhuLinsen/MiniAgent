"""Multi-agent orchestrator for MiniAgent.
...
class Orchestrator:
    """
    Multi-agent orchestrator that decomposes complex tasks into sub-tasks.
    
    Uses a planner agent to break down the task, then spawns worker agents
    for each sub-task. All workers share the same model/key config but get
    different system prompts optimized for their role.
    """

    # Predefined worker roles
    WORKER_PROMPTS = {
        "researcher": "You are a research assistant. Use web_search and bash (curl) to find information. Summarize findings clearly.",
        "coder": "You are a coding assistant. Write clean, well-tested code. Use read/write/edit/bash tools to implement solutions.",
        "tester": "You are a QA engineer. Write and run tests using bash. Report pass/fail results clearly.",
        "reviewer": "You are a code reviewer. Read code carefully and provide constructive feedback on bugs, style, and improvements.",
    }
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.roles = {**self.WORKER_PROMPTS}
        if worker_roles:
